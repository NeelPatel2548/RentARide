...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_POST['car_info']) || !isset($_POST['c_id']) || !isset($_POST['last_booking']) || 
    ...
    // Get car details
    $sqlCarBook = "SELECT * FROM car WHERE car_no = ?";
    $stmt = $conn->prepare($sqlCarBook);
    $stmt->bind_param("s", $car_no);
    $stmt->execute();
    $resultCarBook = $stmt->get_result();
    
    if ($resultCarBook->num_rows === 0) {
        echo "<script>
    ...
    $rowCarBook = $resultCarBook->fetch_assoc();
    $v_id = $rowCarBook['car_no'];
    $carAmount = $rowCarBook['car_price'];
    $a_id = $rowCarBook['agent_id'];

    // Insert booking record
    $sqlBooking = "INSERT INTO booking (c_id, a_id, b_amount, v_id, v_type) VALUES (?, ?, ?, ?, 'Car')";
    $stmt = $conn->prepare($sqlBooking);
    $stmt->bind_param("iids", $c_id, $a_id, $carAmount, $v_id);

    if ($stmt->execute()) {
        // Update car availability
        $sqlAvail = "UPDATE car SET car_avail = 0 WHERE car_no = ?";
        ...
        if ($stmt->execute()) {
            ...
            $sqlUpdateBookedCar = "UPDATE user SET booked_car = ? WHERE user_id = ?";
            $stmt = $conn->prepare($sqlUpdateBookedCar);
            ...
            if ($stmt->execute()) {
                ...
                header('Location: paymentDemo.php');
                exit;
            } else {
                echo "<script>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="car_info" id="car_info" onchange="showCarPreview(this.value)">
                                        ...
                                        while ($carRow = $result->fetch_assoc()) {
                                            $selected = (isset($_POST['car_info']) && $_POST['car_info'] == $carRow['car_no']) ? 'selected' : '';
                                            echo "<option value='" . $carRow['car_no'] . "' " . $selected . ">" . $carRow['car_name'] . "</option>";
                                        }
                                        ?>
                                    </select>
        ...
        <div id="carPreviewPopup" class="popup-form">
            <div class="car-preview-content">
                ...
                <div class="payment-buttons">
                    <button id="closeCarPreview">Close</button>
                </div>
            </div>
        </div>
    </main>
